...
const isExchangeRateApiSuccessResponse = (
  value: ExchangeRateApiSuccessResponse | ExchangeRateApiErrorResponse,
): value is ExchangeRateApiSuccessResponse => {
  return value.result === "success";
};

const isExchangeRateMap = (
  value: unknown,
...
const getLatestRates = async (
  ...
  if (existingSnapshot && isSnapshotFresh(existingSnapshot.fetchedAt)) {
    if (!isExchangeRateMap(existingSnapshot.ratesJson)) {
      throw new Error(
        `Cached exchange rates are invalid for ${existingSnapshot.baseCurrency}`,
      );
    }

    return {
      baseCurrency: existingSnapshot.baseCurrency,
      rates: existingSnapshot.ratesJson,
